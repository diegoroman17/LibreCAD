...
QG_LaserModel::QG_LaserModel(QObject * parent) : QAbstractTableModel(parent) {
...
QG_LaserModel::~QG_LaserModel() {

}

int QG_LaserModel::rowCount ( const QModelIndex & /*parent*/ ) const {
    return listLayer.size();
}

QModelIndex QG_LaserModel::parent ( const QModelIndex & /*index*/ ) const {
    return QModelIndex();
}

QModelIndex QG_LaserModel::index ( int row, int column, const QModelIndex & /*parent*/ ) const {
    if ( row >= listLayer.size() || row < 0)
        return QModelIndex();
    return createIndex ( row, column);
}

bool laserLessThan(const RS_Layer *s1, const RS_Layer *s2) {
     return s1->getName() < s2->getName();
}

void QG_LaserModel::setLaserList(RS_LayerList* ll) {
    listLayer.clear();
    if (ll == NULL)
        return;
    ...
    qSort ( listLayer.begin(), listLayer.end(), laserLessThan );
//called to force redraw
    reset();
}


RS_Layer *QG_LaserModel::getLayer( int row ){
    if ( row >= listLayer.size() || row < 0)
        return NULL;
    return listLayer.at(row);
}

QModelIndex QG_LaserModel::getIndex (RS_Layer * lay){
...
QVariant QG_LaserModel::data ( const QModelIndex & index, int role ) const {
 ...
 * Constructor.
 */
QG_LaserWidget::QG_LaserWidget(QG_ActionHandler* ah, QWidget* parent,
    ...
    laserModel = new QG_LaserModel;
    layerView = new QTableView(this);
    layerView->setModel (laserModel);
    ...
    layerView->setColumnWidth(QG_LaserModel::VISIBLE, 20);
    layerView->setColumnWidth(QG_LaserModel::LOCKED, 20);
 ...
 */
void QG_LaserWidget::setLaserList(RS_LayerList* layerList, bool showByBlock) {
 ...
 */
void QG_LaserWidget::update() {
    RS_DEBUG->print("QG_LaserWidget::update() begin");
    ...
    laserModel->setLaserList(layerList);
 ...
 */
void QG_LaserWidget::activateLaser(RS_Layer* layer) {
    ...
    layerList->activate(layer);
    QModelIndex idx = laserModel->getIndex (layer);
 ...
 */
void QG_LaserWidget::slotActivated(QModelIndex layerIdx /*const QString& layerName*/) {
    if (!layerIdx.isValid() || layerList==NULL) {
        return;
    }

    RS_Layer * lay = laserModel->getLayer( layerIdx.row() );
    if (lay == 0)
        return;

    if (layerIdx.column() == QG_LaserModel::NAME) {
    ...
    layerList->activate(lay);
    if (layerIdx.column() == QG_LaserModel::VISIBLE) {
        actionHandler->slotLayersToggleView();
    }
    if (layerIdx.column() == QG_LaserModel::LOCKED) {
 ...
 */
void QG_LaserWidget::contextMenuEvent(QContextMenuEvent *e) {
 ...
 */
void QG_LaserWidget::keyPressEvent(QKeyEvent* e) {
    switch (e->key()) {
